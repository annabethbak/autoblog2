...
`--dry-run` 을 붙이면 제목/본문만 입력한 뒤 "저장"(임시저장) 버튼만 누르고
끝납니다. "발행" 버튼 관련 로직은 아직 다듬는 중이라, dry-run 에서는 아예
그 경로를 타지 않도록 분리해뒀습니다 — 태그/카테고리/공개설정도 이 모드에선
건드리지 않습니다. 셀렉터를 검증할 때는 이 옵션으로 먼저 확인하세요.
